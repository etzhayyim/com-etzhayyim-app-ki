(ns ki.app
  "com-etzhayyim-app-ki appview — reagent + re-frame, view built from
  ...
  in its <script> block — with two kinds of exception, both made honest
  rather than silently kept:

  - `:app/route-count`, `:app/routes`, and `:app/vars` were `0`, `[]`,
    and `[]` in the Svelte constant — stale placeholders that never
    reflected this repo's actual `wrangler.jsonc` (2 routes,
    `k1k1b001.etzhayyim.com/*` and `ki.etzhayyim.com/*`; 8 declared
    `vars` keys). This port reports what `wrangler.jsonc` actually
    declares instead of carrying the stale zeros forward.
  ...
  - `:app/xrpc?` is now false. This migration's wrangler.jsonc drops
    `main` (see that file's header comment and the repo README): the
    Worker source in this repo does not call `env.ASSETS.fetch`, so the
    XRPC route this page used to advertise as enabled no longer deploys.
    The XRPC handler itself is preserved byte-for-byte at
  ...
  components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-project-ki\"
                         :lang \"ja\"
                         :description \"app-ki — vascular synthesis layer appview (reagent + re-frame + jp-go-dds).\"
                         :css (slurp (io/resource \"jp_go_dds/dds.css\"))}
                        [:div {:id \"app\"} \"etzhayyim-project-ki loading…\"]
...
(def default-db
  {:app/title "Ai etzhayyim Project Ki"
   :app/project "etzhayyim-project-ki"
   :app/name "etzhayyim-project-ki"
   :app/kind "cloudflare surface"
   :app/route-count 2
   :app/routes ["k1k1b001.etzhayyim.com/*" "ki.etzhayyim.com/*"]
   :app/vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_CAPABILITIES" "APP_DESCRIPTION"
              ...
              "APP_UI_TYPE"]
   :app/xrpc? false
   :app/relative-path "cljs/src/ki/app.cljs"})
